...
st.set_page_config(page_title="OnCall Humana - Master 4.9", layout="wide", page_icon="🛡️")
...
with tabs[0]:
    ...
    with st.expander("📥 Importação em Massa (Excel .xlsx)"):
        ...
        if up_file and st.button("🚀 Confirmar Importação"):
            ...
            with conn.session as s:
                ...
                s.commit()
            st.success("Importado com sucesso!"); time.sleep(0.5); st.rerun()
    ...
    with st.form("f_ind", clear_on_submit=True):
        ...
        h, desc = st.number_input("Horas", min_value=0.5, step=0.5), st.text_input("Descrição da tarefa")
...
with tabs[1]:
    ...
    if not meus.empty:
        ...
        c3.metric("Lançamentos Realizados", len(meus))
...
if is_user_admin:
    # === ABA 3: ADMIN APROVAÇÕES (BOTÕES + LIXEIRA) ===
    with tabs[2]:
        st.subheader("🛡️ Gestão de Aprovações")
        st.metric("Total de Linhas no Banco", len(df_lan))
        ...
        c1, c2, c3 = st.columns(3)
        if c1.button("✅ Aprovar TODOS Pendentes", use_container_width=True):
            with conn.session as s:
                s.execute(text("UPDATE lancamentos SET status_aprovaca = 'Aprovado' WHERE status_aprovaca = 'Pendente'"))
                s.commit()
            st.rerun()
            
        if c2.button("💾 Sincronizar Edições Manuais", use_container_width=True):
            with conn.session as s:
                ...
                s.commit()
            st.success("Banco atualizado!"); time.sleep(0.5); st.rerun()
    ...
    # === ABA 4: PAGAMENTOS (DRILL-DOWN RESTAURADO) ===
    with tabs[3]:
        ...
        if not df_ap.empty:
            ...
            for idx, row in df_g.iterrows():
                with st.expander(f"📅 {row['competencia']} | 👤 {row['colaborador_email']} | 💰 R$ {row['total_r$']:,.2f}"):
                    ...
                    c_status, c_act = st.columns([3, 1])
                    status_atual = det['status_pagamento'].iloc[0] if 'status_pagamento' in det.columns else "Em aberto"
                    novo_s = c_status.selectbox(f"Mudar Status ({idx})", ["Em aberto", "Pago", "Parcial"], index=["Em aberto", "Pago", "Parcial"].index(status_atual if status_atual in ["Em aberto", "Pago", "Parcial"] else "Em aberto"))
                    
                    if c_act.button(f"Confirmar {idx}", use_container_width=True):
                        with conn.session as s:
    ...
    with tabs[4]:
        ...
        st.write("**Ranking Financeiro por Colaborador**")
    ...
    # === ABA 6: CONFIGURAÇÕES (BANCOS + PERMISSÕES) ===
    with tabs[5]:
...
# --- FOOTER ---
st.markdown("---")
st.markdown("<p style='text-align: center; color: gray;'>OnCall Humana by Pedro Reis | v4.9 Master Enterprise</p>", unsafe_allow_html=True)
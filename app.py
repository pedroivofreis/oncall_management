...
def get_connection():
    try:
        ...
        st.error("Erro ao conectar ao banco Neon. Verifique o status no Console.")
        st.stop()

conn = get_connection()

# --- 3. FUNÇÕES DE CARREGAMENTO (SEM CACHE PARA SEGURANÇA) ---
...
with tabs[1]:
    ...
    if not meus.empty:
        ...
        m1.metric("Horas Totais", f"{meus['horas'].sum():.1f}h")
        m2.metric("Valor a Receber", f"R$ {meus['total_r$'].sum():,.2f}")
        m3.metric("Lançamentos", len(meus))
...
if is_user_admin:
    ...
    with tabs[3]:
        ...
        if not df_pag.empty:
            ...
            comp = st.selectbox("Mês:", sorted(df_pag['competencia'].unique(), reverse=True))
            df_m = df_pag[df_pag['competencia'] == comp]
            p1, p2 = st.columns(2)
            p1.metric("Pendente (Mês)", f"R$ {df_m[df_m['status_pagamento'] != 'Pago']['total_r$'].sum():,.2f}")
    ...
    with tabs[4]:
        ...
        s1.metric("Linhas", len(df_bi))
        s2.metric("Horas", f"{df_bi['horas'].sum():.1f}h")
        s3.metric("Custo Total", f"R$ {df_bi['custo'].sum():,.2f}")
        s4.metric("Pago Total", f"R$ {df_bi[df_bi['status_pagamento'] == 'Pago']['custo'].sum():,.2f}")
        c1, c2 = st.columns(2)
        with c1: st.bar_chart(df_bi.groupby("projeto")["custo"].sum()); st.write("Custo/Projeto")
        with c2: st.bar_chart(df_bi.groupby("colaborador_email")["horas"].sum()); st.write("Horas/Colaborador")
...
# --- FOOTER ---
st.markdown("---")
st.markdown("<p style='text-align: center; color: gray;'>by Pedro Reis | Versão 3.0.7 Enterprise</p>", unsafe_allow_html=True)
...
st.set_page_config(page_title="OnCall Humana - Master v5.4", layout="wide", page_icon="🛡️")
...
# --- 5. NAVEGAÇÃO E ESTADO (O SEGREDO DO REFRESH) ---
st.sidebar.divider()
st.sidebar.subheader("📍 Navegação")

# Definimos as opções baseadas no perfil
if is_user_admin:
...
# O selectbox no sidebar segura o estado mesmo após o rerun
...
# --- SEÇÃO ADMIN ---
    ...
    with st.expander("📥 IMPORTAÇÃO RÁPIDA (Copiar e Colar do Excel)"):
        st.write("Copie as colunas: **Data | Projeto | E-mail Usuário | Horas | Tipo | Descrição**")
    ...
    df_adm_v = df_lan.copy(); df_adm_v.insert(0, "🗑️", False)
    ...
    if not df_ap.empty:
        ...
        for idx, row in df_g.iterrows():
            with st.expander(f"📅 {row['competencia']} | 👤 {row['colaborador_email']} | 💰 R$ {row['total_r$']:,.2f}"):
                ...
                n_s = st.selectbox(f"Status Pagamento ({idx})", ["Em aberto", "Pago", "Parcial"], key=f"s_{idx}")
    ...
    df_bi = df_lan.copy(); df_bi["custo"] = df_bi["horas"] * df_bi["valor_hora_historico"]
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Registros", len(df_bi)), k2.metric("Horas", f"{df_bi['horas'].sum():.1f}h"), k3.metric("Custo Total", f"R$ {df_bi['custo'].sum():,.2f}"), k4.metric("Já Pago", f"R$ {df_bi[df_bi['status_pagamento'] == 'Pago']['custo'].sum():,.2f}")
...
# --- FOOTER ---
st.markdown("---")
st.markdown("<p style='text-align: center; color: gray;'>OnCall Humana by Pedro Reis | v5.4 Master Enterprise</p>", unsafe_allow_html=True)